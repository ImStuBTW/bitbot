...
app.post('/:part0/:part1', function (req, res) {
  ...
  if (req.body.push != undefined) {
    for (let i = 0; i < req.body.push.changes[0].commits.length; i++) {
      ...
      message.commit_link = req.body.push.changes[0].links.html.href;
      message.branch = req.body.push.changes[0].old.name;

      pushPost(message);
    }
  } else if (req.body.pullrequest != undefined) {
    message.pr = req.body.pullrequest.description;
    message.pr_link = req.body.pullrequest.links.html.href;
